...
const SHARE_TITLE = "奶呱壁纸高清搞笑抽象手机可保存";
const SHARE_PATH = "/pages/wallpaper/wallpaper";
const DOWNLOAD_DIALOG_EXIT_DURATION = 260;
const PHOTO_ALBUM_DENIED_CONTENT = "你已关闭保存到相册权限，请在设置中重新开启后再保存壁纸。";
const SHARE_DOWNLOAD_CATEGORY_KEY = "wallpaper";
...
Page({
  data: {
    ...
    pageIntroTitle: "奶呱壁纸大全",
    pageIntroText: "精选奶呱壁纸、搞笑壁纸、抽象壁纸合集，支持预览后保存，适合手机桌面和锁屏背景使用。",
    pageIntroTags: ["壁纸大全", "手机壁纸", "搞笑壁纸"],
  },
  ...
  onShareAppMessage() {
    const imageUrl =
      ...
      this.data.selectedWallpaperImage ||
      getFirstMediaImage(this.data.mediaGroups) ||
      RESOURCE_LINKS.wallpaperImageUrl;
  ...
  loadRemoteMediaGroups() {
    wx.request({
      url: RESOURCE_LINKS.wallpaperPagesJsonUrl,
      success: (res) => {
        const remoteGroups = normalizeRemoteGroups(res.data, "壁纸");
  ...
  handleRewardSaveFeaturedTap(event) {
    ...
    this.setData({
      ...
      selectedWallpaperImage: image,
      selectedGroupImages: groupImages,
      shareDownloadHintText: "看完视频后可保存当前这组壁纸。",
    });
  },

  handleRewardSaveCurrentTap() {
    const image = this.data.selectedWallpaperImage;
  ...
  applyShareDownloadState(limitState) {
    ...
    const hintText = isDisabled
      ? `分享保存今日已用完，明天会自动恢复 ${DAILY_SHARE_DOWNLOAD_LIMIT} 次；看视频可保存当前这组壁纸。`
      : `分享保存今日还可用 ${remainingCount} 次，用完后明天恢复；看视频可保存当前这组壁纸。`;
