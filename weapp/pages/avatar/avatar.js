...
const SHARE_TITLE = "奶呱头像大全搞笑抽象高清可保存";
const SHARE_PATH = "/pages/avatar/avatar";
const DOWNLOAD_DIALOG_EXIT_DURATION = 260;
const PHOTO_ALBUM_DENIED_CONTENT = "你已关闭保存到相册权限，请在设置中重新开启后再保存头像。";
const SHARE_DOWNLOAD_CATEGORY_KEY = "avatar";

function normalizeAvatarGroupItems(items) {
  return (items || [])
    .map((item) => {
...
function normalizeRemoteAvatarGroups(groups) {
  ...
  return groups
    .map((group, groupIndex) => {
      const items = normalizeAvatarGroupItems(group?.items);
      ...
      return {
        title: group?.title || `精选头像${groupIndex + 1}组`,
        items,
      };
    })
    .filter(Boolean);
...
function buildAvatarGroups(groups) {
  return (groups || [])
    .map((group, groupIndex) => {
      ...
      return {
        title: group?.title || `头像组 ${groupIndex + 1}`,
...
function getFirstAvatarImage(groups) {
  return groups?.[0]?.featuredImage || "";
}

function getAvatarGroupImagesByImage(groups, image) {
  if (!image) {
    return [];
  }
...
Page({
  data: {
    ...
    pageIntroTitle: "奶呱头像大全",
    pageIntroText: "精选奶呱头像、搞笑头像、抽象头像合集，支持预览后保存，适合微信头像和社交平台头像使用。",
    pageIntroTags: ["头像大全", "微信头像", "搞笑头像"],
  },
  ...
  onShareAppMessage() {
    const imageUrl =
      ...
      this.data.selectedAvatarImage ||
      getFirstAvatarImage(this.data.mediaGroups) ||
      RESOURCE_LINKS.avatarImageUrl;
  ...
  loadRemoteMediaGroups() {
    wx.request({
      url: RESOURCE_LINKS.avatarPagesJsonUrl,
      success: (res) => {
        const remoteGroups = normalizeRemoteAvatarGroups(res.data);

        this.setData({
          mediaGroups: buildAvatarGroups(remoteGroups),
  ...
  handleRewardSaveFeaturedTap(event) {
    ...
    const groupImages = getAvatarGroupImagesByImage(this.data.mediaGroups, image);
    this.setData({
      ...
      shareDownloadHintText: "看完视频后可保存当前这组头像。",
    });
  },

  handleRewardSaveCurrentTap() {
    const image = this.data.selectedAvatarImage;
    ...
    this.pendingRewardDownloadGroupImages = getAvatarGroupImagesByImage(this.data.mediaGroups, image);
    this.closeDownloadDialog();
  ...
  applyShareDownloadState(limitState) {
    ...
    const hintText = isDisabled
      ? `分享保存今日已用完，明天会自动恢复 ${DAILY_SHARE_DOWNLOAD_LIMIT} 次；看视频可保存当前这组头像。`
      : `分享保存今日还可用 ${remainingCount} 次，用完后明天恢复；看视频可保存当前这组头像。`;
