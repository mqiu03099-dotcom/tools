  ...
  <div class="page">
    ...
    <div class="stack-layout">
      <section class="panel">
        ...
        <div class="import-grid">
          <label class="upload-card">
            <span>本地上传</span>
            <strong>{{ localFileName || uploadLabel }}</strong>
          ...
          <div class="input-card">
            <label :for="`remote-url-${kind}`">外部素材链接</label>
            <textarea
              :id="`remote-url-${kind}`"
      ...
      <section class="panel output-panel">
        <div class="panel-header">
          <h2>结果输出</h2>
          <span>{{ imageMeta.fileName || "等待导入素材" }}</span>
        </div>
...
const props = withDefaults(
  defineProps<{
    kind: "avatar" | "wallpaper" | "emojis" | "audio";
    title: string;
    jsonFileName: string;
  ...
  }>(),
  {
    mediaKind: "image",
    eyebrow: "Media Admin",
    description: "支持本地上传和外链下载到 ",
    uploadHint: "支持常见图片类型，上传后自动写入 public 目录",
    uploadLabel: "选择一个素材文件",
...
function syncJsonEditorHeightLater() {
...
async function loadJson() {
  ...
  try {
    const response = await $fetch<{ content: string }>(`/api/asset-admin/${props.kind}/json`);
    jsonText.value = response.content;
    await nextTick();
    syncJsonEditorHeightLater();
    setStatus(`已加载 ${props.jsonFileName}`, "success");
  } catch (error: any) {
    setStatus(error?.data?.statusMessage || error?.message || "读取 JSON 失败", "error");
  } finally {
...
async function uploadLocalFile(file: File) {
  ...
  formData.append("mediaKind", props.mediaKind);
  ...
  setStatus("本地素材已保存到 public 目录", "success");
}
...
async function handleFileChange(event: Event) {
  ...
  const expectedPrefix = props.mediaKind === "audio" ? "audio/" : "image/";

  if (!file.type.startsWith(expectedPrefix)) {
    setStatus(props.mediaKind === "audio" ? "请选择音频文件" : "请选择图片文件", "error");
    input.value = "";
    return;
  }
  ...
  try {
    ...
    setStatus(error?.data?.statusMessage || error?.message || "素材上传失败", "error");
  } finally {
    input.value = "";
  }
}
...
async function importRemoteImage() {
  ...
  try {
    ...
    applyImageMeta(response);
    setStatus("外链素材已下载并保存到 public 目录", "success");
  } catch (error: any) {
    setStatus(error?.data?.statusMessage || error?.message || "远程素材导入失败", "error");
  } finally {
